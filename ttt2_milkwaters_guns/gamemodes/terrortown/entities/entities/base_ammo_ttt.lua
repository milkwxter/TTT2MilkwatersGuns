...
ENT.M_AmmoName = "Base ammo"
ENT.M_CookingOff = false
...
function ENT:OnTakeDamage(dmginfo)
	-- prevent shooting the same box multiple times
	if self.M_CookingOff == true then return end
	
	-- get random number of time to cook off
	local randomNumber = math.random(6, 12)
	
	-- run randomNumber amount of times
    for i = 0, randomNumber do
		-- prevent shooting the same box multiple times
		self.M_CookingOff = true
		
		-- delay the next step by 0.4 seconds
        timer.Simple(i * 0.4, function()
			-- cook the ammo box a little
            AmmoCook(self)
			-- remove it on the last loop
			if i == randomNumber and IsValid(self) then
				self:Remove()
			end
        end)
    end
...
function AmmoCook(self)
	-- make sure we are valid
	if not IsValid(self) then return end
	
	-- make ammo bounce around
	local phys = self:GetPhysicsObject()
